...
1. **Define the spec.** Before writing any code, create or update the relevant spec in `docs/specs/`. Use the `_template.md` in each directory as a starting point.
...
| | ADRs (`docs/adr/`) | Specs (`docs/specs/`) |
|---|---|---|
...
**Specs reference ADRs, never the other way around.** For example, the event entity spec defines its specific fields (name, date, description). It does not repeat that every table has `created_at`, `updated_at`, and `deleted_at` — that's an ADR ([ADR-007](docs/adr/api/007-paranoid-soft-delete.md)) that applies to all entities.
...
| Directory | Scope |
|-----------|-------|
| `docs/adr/` | Global (monorepo-wide) |
| `docs/adr/api/` | API-specific |
| `docs/adr/site/` | Site-specific |
...
- **Soft delete** on all entities via `deleted_at` column (see [ADR-007](docs/adr/api/007-paranoid-soft-delete.md)).
